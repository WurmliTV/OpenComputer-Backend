...
local function pullSignal(timeout)
    ...
    if bootloader and ( signal[1]:match("ey") and not bootloader:match(signal[5]) or signal[1]:match("cl") and not bootloader:match(signal[4]) ) then
        return ""
    end
    ...
    if proxy and address ~= computer.tmpAddress() then
        ...
        bootCandidates[i] = {
            ...
            p = function(booting, y)
                ...
                booting = booting and not userChecked and bootloader and (bootloader:match("+") and pcall(status, "Hold ENTER to boot", F, math.huge, 28))
            end
        }
